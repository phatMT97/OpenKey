...
- [ ] C++: Add `--dialogname` router in `main.cpp`
- [ ] C++: Add spawn code in `AppDelegate.cpp` (with single-instance check)
...
LRESULT CALLBACK DialogName::SubclassProc(HWND hwnd, UINT msg, WPARAM wParam, 
    ...
    if (msg == WM_NCHITTEST) {
        ...
        if (result == HTCLIENT) {
            ...
            int closeButtonZone = winRect.right - 40;  // Exclude close button area
            
            if (pt.y < 40 && pt.x < closeButtonZone) {
                return HTCAPTION;  // Enable drag in title bar
            }
        }
        return result;
    }
...
```cpp
// In main.cpp, add before engine code:
if (lpCmdLine && wcsstr(lpCmdLine, L"--dialogname")) {
    // Enable Inspector
...
## 4. AppDelegate.cpp - Spawn with Single-Instance Check

```cpp
void AppDelegate::openDialogName() {
    // Prevent multiple instances
    HWND existingWnd = FindWindow(NULL, _T("Dialog Title"));
    if (existingWnd) {
...
| Single instance | `FindWindow()` check before `CreateProcess()` |
